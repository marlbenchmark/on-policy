...
# Five-training-seed comparison on the original, seen-condition validation grid.
...
EVAL_START=60000
EVAL_EPISODES=200
LOG_DIR="$RESULTS/robust_baseline_eval_logs_5seed"
...
for train_seed in 1 2 3 4 5; do
  ...
  if [[ -s "$output/episode_results.csv" && -s "$output/summary.json" ]]; then
    echo "SKIP complete robust evaluation trainseed${train_seed}"
    continue
  fi
  [[ ! -e "$output" ]] || {
...
echo "ROBUST_BASELINE_EVAL_5SEED_COMPLETE"
